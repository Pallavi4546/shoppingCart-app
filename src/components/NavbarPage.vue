<template>
  <div>
    <nav class="navbar navbar-expand-md navbar-white bg-white  ">
      ...
      <button class="navbar-toggler" type="button" @click.prevent="isNavbarOpen = !isNavbarOpen"
      ...
      <div class="collapse navbar-collapse" :class="{ 'show': isNavbarOpen }">
        <ul class="navbar-nav mx-4 ">
          ...
          <li class="nav-item" v-if="login">
            <router-link class="nav-link " to="/cart-items"><button class="btn p-2"><button
                  :style="cartCount ? `width:25px;height:25px;background:white;border-radius:10px;border:none` : `display:none`">{{
                    cartCount
                  }}</button> View Cart </button></router-link>
          </li>
          <li class="nav-item" v-if="login">
            <router-link class="nav-link " to="#"><button class="btn p-2" @click.prevent="logout()">Log
